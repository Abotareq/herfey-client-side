...
export default function OrderDetailsPage() {
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-50 via-white to-orange-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-red-600">Unable to fetch order details. Please try again.</p>
        </div>
      </div>
    )
  }
  ...
  const statusOptions = [
    ...
    { value: 'ready', label: 'Ready for Pickup/Delivery', icon: '📦' },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-orange-100 p-8 hover:shadow-xl transition-all duration-300">
            ...
            <div className="space-y-2 text-gray-700">
              ...
              <p><span className='font-semibold text-orange-800'>City</span> : {order.shippingAddress?.city}, {order.shippingAddress?.state}</p>
              <p><span className='font-semibold text-orange-800'>Postal Code</span> : {order.shippingAddress?.postalCode}</p>
              <p className="text-sm text-gray-600 mt-3">
                📝 Notes: {order.shippingNotes || 'No special instructions'}
              </p>
            </div>
          </div>
        </div>
