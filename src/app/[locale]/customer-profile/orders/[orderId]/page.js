...
import { useCancelOrder, useGetUserOrderById } from '@/service/customerOrderService';
        ...
        <div className="bg-white/60 backdrop-blur-lg rounded-2xl p-6 border border-white/20 shadow-md">
            <ol className="grid grid-cols-2 sm:grid-cols-4 gap-6 text-sm font-medium text-slate-500">
                {statuses.slice(1).map((s, index) => {
                    ...
                    return (
                        <li key={s} className="flex items-center gap-2 transition-colors duration-300">
                             <span className={isActive ? 'text-orange-500' : 'text-slate-400'}>{Icons.checkmark}</span>
                             <span className={isActive ? 'text-slate-800 font-semibold' : ''}>{getStatusLabel(s)}</span>
                        </li>
                    );
                })}
            </ol>
...
const OrderItemsCard = ({ items, t }) => (
    <div className="lg:col-span-2 bg-white/60 backdrop-blur-lg rounded-2xl p-6 border border-white/20 shadow-md space-y-4">
        <div className="flex items-center gap-3">
            ...
            <h4 className="font-semibold text-slate-800 text-lg">{t('itemsInOrder')} ({items.length})</h4>
        </div>
        {items.map((item) => (
...
// --- Order Sidebar ---
const OrderSidebar = ({ order, isCancellable, isCancelling, onCancelClick, t, tOrders }) => (
    <div className="space-y-6">
        <div className="bg-white/60 backdrop-blur-lg rounded-2xl p-6 border border-white/20 shadow-md">
            <div className="flex items-center gap-3 mb-4">
                <span className="text-orange-500">{Icons.location}</span>
                <h4 className="font-semibold text-slate-800 text-lg">{t('shippingAddress')}</h4>
            </div>
        ...
        <div className="bg-white/60 backdrop-blur-lg rounded-2xl p-6 border border-white/20 shadow-md">
            <div className="flex items-center gap-3 mb-4">
                ...
                <h4 className="font-semibold text-slate-800 text-lg">{t('orderSummary')}</h4>
            </div>
            <div className="space-y-2 text-slate-600">
                <div className="flex justify-between"><span>{t('subtotal')}</span><span>${order.subtotal.toFixed(2)}</span></div>
                <div className="flex justify-between"><span>{t('shippingFee')}</span><span>${order.shippingFee.toFixed(2)}</span></div>
                <div className="flex justify-between"><span>{t('tax')}</span><span>${order.tax.toFixed(2)}</span></div>
                <div className="flex justify-between font-bold text-slate-800 text-lg pt-2 border-t border-slate-200/50 mt-2"><span>{t('total')}</span><span>${order.totalAmount.toFixed(2)}</span></div>
            </div>
        </div>
...
function OrderDetailsPage () {
    ...
    const { data: order, isLoading, isError, error, refetch } = useGetUserOrderById(orderId);
    ...
    const { mutate: cancelOrder, isLoading: isCancelling } = useCancelOrder({
        ...
        onError: (err) => {
            toast.error(err.response?.data?.error || tOrders('orderCancellationFailed'));
        }
    });
        ...
        <div className="min-h-screen bg-gradient-to-br from-slate-50 to-gray-200 p-4 sm:p-6 md:p-8">
            <div className="max-w-7xl mx-auto space-y-6">
                <div className="flex items-center gap-4">
                    <Link href="/customer-profile" className="text-slate-500 hover:text-orange-600 transition-colors">
                        {Icons.back}
                    </Link>
                    <div>
                        <h3 className="text-2xl font-bold text-slate-900">{t('title')}</h3>
                        <p className="text-slate-600">Order #{order._id.slice(-8)} &bull; Placed on {new Date(order.createdAt).toLocaleDateString()}</p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <OrderSidebar order={order} isCancellable={isCancellable} isCancelling={isCancelling} onCancelClick={handleCancelClick} t={t} tOrders={tOrders} />
                </div>
            </div>
        </div>
    );
