  ...
  return (
    <div className="space-y-6 ">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
        <div>
          <h3 className="text-2xl font-bold text-gray-900">{t('myOrders')}</h3>
        ...
        {ordersData?.orders?.length > 0 ? (
          ...
          ordersData.orders.map((order) => (
            <div key={order._id} className="bg-white rounded-2xl p-6 shadow-xs hover:shadow-lg transition-all duration-300">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4">
                <div>
                  <p className="text-sm text-gray-500">{t('order')} #{order._id.slice(-6)}</p>
              ...
              <div className="flex -space-x-4">
                ...
                {order.orderItems.length > 5 && (
                  <div className="w-12 h-12 rounded-full bg-gray-200 flex items-center justify-center text-sm font-semibold text-gray-600 border-2 border-white">
                    +{order.orderItems.length - 5}
                  </div>
                )}
              </div>
          ...
          !isLoading && (
            <div className="text-center py-12 bg-gray-50 rounded-lg">
              ...
              <p className="text-gray-600">{t('noOrdersSubtext')}</p>
            </div>
          )
        )}
      </div>
      ...
      {ordersData?.pages > 1 && (
        <div className="flex justify-center items-center gap-4 mt-6">
          <button onClick={() => setPage(p => Math.max(p - 1, 1))} disabled={page === 1} className="px-4 py-2 border border-gray-300 rounded-lg disabled:opacity-50">
            {t('previous')}
          </button>
          <span>{t('pageInfo', { page: ordersData.page, pages: ordersData.pages })}</span>
          <button onClick={() => setPage(p => (ordersData.page < ordersData.pages ? p + 1 : p))} disabled={ordersData.page === ordersData.pages} className="px-4 py-2 border border-gray-300 rounded-lg disabled:opacity-50">
            {t('next')}
          </button>
        </div>
      )}
